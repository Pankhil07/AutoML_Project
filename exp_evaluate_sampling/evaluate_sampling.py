...
if __name__ == '__main__':
    ...
    for seed in range(num_seeds):
        ...
        results['LLAMBO']['proposed_points_evaluation'].append(candidate_scores.tolist())
                ...
                # Setup task context and LLM sampler
        task_context = {
            'model': model,
            'task': task_type,
            'tot_feats': dataset['train_x'].shape[1],
            'cat_feats': 0,
            'num_feats': dataset['train_x'].shape[1],
            'n_classes': len(np.unique(dataset['train_y'])),
            'metric': task_metric,
            'num_samples': dataset['train_x'].shape[0],
            'hyperparameter_constraints': hp_constraints
        }

        # Initialize LLM_ACQ sampler
        LLM_Sampler = LLM_ACQ_LMX(
        ...
        # Sample candidates
        candidates = asyncio.run(LLM_Sampler.acquire(observed_configs, observed_fvals, alpha=-0.2))
        candidates_df = pd.DataFrame(candidates)

        # Evaluate candidates
        candidates_dicts = candidates_df.to_dict('records')
        evaluation = evaluate_proposals(
            candidates_dicts, observed_configs, model, task_type, task_metric, 
            dataset, lower_is_better, hp_constraints
        )

        av_regret, best_regret, perf_spread, ml_dist, gen_var, ll, candidate_scores = evaluation
        logger.info(f'[LMX] Average regret: {av_regret:.4f}, Best regret: {best_regret:.4f}, '
                    f'Score spread: {perf_spread:.4f}, Mahalanobis distance: {ml_dist:.4f}, '
                    f'Gen var: {gen_var:.4f}, Log likelihood: {ll:.4f}')
        results['LMX']['av_regret'].append(av_regret)
        results['LMX']['best_regret'].append(best_regret)
        results['LMX']['score_spread'].append(perf_spread)
        results['LMX']['ml_dist'].append(ml_dist)
        results['LMX']['gen_var'].append(gen_var)
        results['LMX']['ll'].append(ll)
        results['LMX']['proposed_points'].append(candidates_dicts)
        results['LMX']['proposed_points_evaluation'].append(candidate_scores.tolist())
        ...
        logger.info('='*200)
        logger.info(f'[LMX] {seed+1} evaluation runs complete! Total cost: ${tot_llm_cost:.4f}')
